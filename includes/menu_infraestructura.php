...
try {
    ...
    if ($tiene_dep && $id_tecnico) {
        $stmt_disponibles = $conn->prepare("SELECT COUNT(*) as total FROM Tickets t
            ...
            WHERE t.estado = 'Nuevo' AND t.oati_asignado IS NULL AND t.area_tipo = 'infraestructura'
            AND d.departamento_infraestructura_id IN (SELECT departamento_id FROM DepartamentoTecnicos WHERE usuario_id = ?)");
        $stmt_disponibles->execute([$id_tecnico]);
    } else {
        // Si no tiene departamento, mostrar todos los de infraestructura
