...
// Verificar que sea una petición POST y que el usuario sea OATI
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || $_SESSION['privilegio'] !== 'oati') {
...
$ticket_id = intval($_POST['ticket_id']);
             ...
             $stmt = $conn->prepare("SELECT estado, oati_asignado FROM tickets WHERE id = ?");
             ...
             if ($ticket['estado'] !== 'nuevo') {
                 ...
                 // Asignar ticket al OATI
                 $stmt = $conn->prepare("UPDATE tickets SET estado = 'asignado', oati_asignado = ?, fecha_asignacion = NOW() WHERE id = ?");
                 $stmt->bind_param("ii", $oati_id, $ticket_id);
                
if ($stmt->execute()) {
                     // Registrar en historial
                     $stmt2 = $conn->prepare("INSERT INTO historial_tickets (ticket_id, usuario_id, accion, detalles) VALUES (?, ?, 'asignado', 'Ticket asignado al OATI')");
                     $stmt2->bind_param("ii", $ticket_id, $oati_id);
                     $stmt2->execute();
        ...
        case 'cambiar_estado':
            ...
            if (!in_array($nuevo_estado, $estados_permitidos)) {
             ...
             // Verificar que el OATI sea el asignado
             ...
             if ($ticket['oati_asignado'] != $oati_id) {
        ...
        case 'agregar_comentario':
            ...
            if (empty($comentario)) {
             ...
             $stmt->bind_param("iis", $ticket_id, $oati_id, $comentario);
             
             if ($stmt->execute()) {
                 // Registrar en historial
                 $stmt2 = $conn->prepare("INSERT INTO historial_tickets (ticket_id, usuario_id, accion, detalles) VALUES (?, ?, 'comentario', 'Comentario OATI agregado')");
                 $stmt2->bind_param("ii", $ticket_id, $oati_id);
                 $stmt2->execute();
